...
def spec_scenario(spec, testcase, generations=0, pop_size=1,file_directory=None):
    ...
    loop.run_until_complete(
        asyncio.gather(hello(testcase, scenario_specification, generation_number=generations, population_size=pop_size,
...
def test_scenario(direct, item):
    file = direct + item
    log_direct = "model_coverage_3/" + Path(item).stem
    ...
    case_directory = 'model_coverage_test_cases/'
    case_file = case_directory + Path(item).stem + '/all_case_1.json'
    with open(case_file) as f:
        test_cases_dic = json.load(f)
        test_cases = test_cases_dic['cases']
    ...
    for i in range(len(testcases)):
        ...
        try:
            ...
            for spec_index in range(len(specifications_in_scenario)):
                ...
                spec_scenario(spec=single_specification, testcase=test_cases, generations=25, pop_size=20,
